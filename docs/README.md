# Project Documentation

Guides for building this Flutter movie app with Clean Architecture.

| Document | Description |
|----------|-------------|
| **[implementation-plan.md](./implementation-plan.md)** | **Master roadmap — all phases, tasks, checkpoints, current progress** |
| [architecture.md](./architecture.md) | Clean Architecture layers, dependency rules, data flow |
| [folder-structure.md](./folder-structure.md) | Directory tree, naming conventions, feature checklist |
| [tech-stack.md](./tech-stack.md) | Libraries: Cubit, easy_localization, Retrofit, Hive, storage |
| [development-workflow.md](./development-workflow.md) | Step-by-step feature development, commits, migration |
| [boilerplate-templates.md](./boilerplate-templates.md) | Copy-paste templates for each layer |
| [reference-repo.md](./reference-repo.md) | **Local reference clone path** (design source) |

## Reference app (local clone)

```
/Users/karimelbahi/Programming/Android/Study/Flutter/flutter-tmdbmovie-bloc-cubit
```

See [reference-repo.md](./reference-repo.md) for file mapping and refresh commands.

## Cursor AI Setup

- **Skill:** `.cursor/skills/flutter-movie-app/SKILL.md` — agent workflow for this project
- **Rules:** `.cursor/rules/` — enforced conventions during development

When starting a new feature, read `architecture.md` and `development-workflow.md` first.
