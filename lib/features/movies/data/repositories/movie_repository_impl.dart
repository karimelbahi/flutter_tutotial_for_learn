...
class MovieRepositoryImpl implements MovieRepository {
  ...
  Future<Result<List<Movie>>> _fetchMovies(
    ...
    try {
      ...
      // Retrofit wraps our interceptor exceptions inside DioException.error
      final cause = error.error;
      if (cause is NetworkException) {
        return Error(NetworkFailure(cause.message));
      }
      if (cause is ServerException) {
        return Error(ServerFailure(cause.message));
      }
      return Error(NetworkFailure(error.message ?? 'Network error occurred'));
